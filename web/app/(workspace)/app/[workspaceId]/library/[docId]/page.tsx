...
export default function LibraryDocPage() {
  ...
  const { notes: pageNotes, loading: notesLoading } = useRecapNotes(
    workspaceId,
    doc?.pageUrl ?? undefined,
  )

  const recapDoc = useMemo((): FolderDocument | null => {
    if (!doc) return null
    return {
      id: doc.id,
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
      ...
      <div className="relative flex min-h-0 flex-1 overflow-hidden">
        <div className="min-h-0 flex-1 overflow-y-auto scrollbar-minimal">
          <div className="max-w-3xl mx-auto pt-6 pb-40" style={{ paddingLeft: 80, paddingRight: 80 }}>
          {doc.autoGenerated && (
          ...
          <FolderDocumentEditor
            ...
            className={doc.autoGenerated ? 'auto-recap-document' : undefined}
            recapMode={doc.autoGenerated}
          />

          {doc.autoGenerated && !notesLoading && pageNotes.length > 0 && (
        ...
        {doc.autoGenerated && recapDoc && (
          <RecapContextPanel
            doc={recapDoc}
            notes={pageNotes}
            contentHtml={doc.content}
            loadingNotes={notesLoading}
            onRegenerate={() => { /* server-backed; auto-regenerates */ }}
