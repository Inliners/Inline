...
function ActivitySummaryCard({ data }: { data: DashboardStats['captureHistory'] }) {
  const total = data.reduce((sum, item) => sum + item.count, 0)
  const activeDays = data.filter(item => item.count > 0).length
  const busiest = data.reduce((best, item) => (item.count > best.count ? item : best), data[0] ?? { date: '', count: 0 })
  const busiestLabel = busiest.date
    ? new Date(busiest.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
    : 'No activity'
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-[#263E7A] dark:bg-[#15285C]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-slate-800 dark:text-white">Activity rhythm</p>
          <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-[#9BBCE5]">
            A quick read on how consistently this workspace is being captured.
          </p>
        </div>
        <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-medium text-slate-600 dark:border-[#263E7A] dark:bg-[#1B326D] dark:text-[#C9DAF0]">
          30 days
        </span>
      </div>
      <div className="mt-5 grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Active days', value: `${activeDays}/${data.length}` },
          { label: 'Best day', value: busiest.count > 0 ? `${busiest.count} on ${busiestLabel}` : 'None yet' },
        ].map(item => (
          <div key={item.label} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 dark:border-[#263E7A] dark:bg-[#1B326D]">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-[#9BBCE5]">{item.label}</p>
            <p className="mt-1 truncate text-sm font-semibold text-slate-800 dark:text-white">{item.value}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      <section id="activity" className="scroll-mt-28 space-y-4">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(380px,0.85fr)] items-start">
          <div className="min-w-0 space-y-5">
            <ActivityHeatmap data={stats.captureHistory} />
            <ActivitySummaryCard data={stats.captureHistory} />
          </div>
